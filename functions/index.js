...
const match = (userHandType, conv) => {
  ...
  // アプリの手を決定
  const appHandType = constants.VALID_HAND_TYPE[Math.floor(Math.random() *
    constants.VALID_HAND_TYPE.length)];
  // 勝敗を決定
  const matchType = constants.MATCH_RESULT[userHandType][appHandType];

  // 勝敗別のメッセージを返す
  const resultMessage =
  constants.SPEAK_MESSAGE_RESULT[matchType].replace("%HAND", appHandType);
...
app.intent(constants.INTENT_NAME_MATCH, (conv) => {
  // ユーザーの手を取得
  const userHandType = conv.parameters["handType"];
